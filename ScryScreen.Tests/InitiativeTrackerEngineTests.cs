...
public class InitiativeTrackerEngineTests
{
    ...
    public void Add_NormalizesEmptyName_AndSetsActiveIfMissing()
    {
        ...
        var entry = new InitiativeEntry(Guid.NewGuid(), "   ", 12);
    ...
    {
        var a = new InitiativeEntry(Guid.NewGuid(), "A", 10);
        var b = new InitiativeEntry(Guid.NewGuid(), "B", 5);
    ...
    [Fact]
    public void Sort_SortsByInitiativeDesc_ThenName_ThenId()
    {
        ...
        var a = new InitiativeEntry(id1, "zeta", 10);
        var b = new InitiativeEntry(id2, "Alpha", 10);
        var c = new InitiativeEntry(id3, "Alpha", 10);

        var state = new InitiativeTrackerState(new[] { a, b, c }, Round: 1, ActiveId: a.Id);
        state = InitiativeTrackerEngine.Sort(state);

        Assert.Equal(new[] { b.Id, c.Id, a.Id }, state.Entries.Select(e => e.Id).ToArray());
    ...
    public void Remove_RemovesEntry_AndIfActiveRemoved_SelectsNext()
    {
        var a = new InitiativeEntry(Guid.NewGuid(), "A", 10);
        var b = new InitiativeEntry(Guid.NewGuid(), "B", 5);
        var c = new InitiativeEntry(Guid.NewGuid(), "C", 1);
